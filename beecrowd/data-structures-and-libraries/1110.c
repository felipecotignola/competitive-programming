...
int remov(Deck* deck){
    int resp=(*deck).remainingCards[(*deck).top++%(*deck).capacity];
    (*deck).quantity--;
    (*deck).remainingCards[++(*deck).bottom%(*deck).capacity]=(*deck).remainingCards[(*deck).top];
    (*deck).top=++(*deck).top%(*deck).capacity;
    return resp;
}
void loop(Deck* deck){
